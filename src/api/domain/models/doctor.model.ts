...
export const getDoctorDetailsBySlug = async (
    ...
    try {
        ...
        const response = {
            ...
            overviewParagraphs: [doctor.about || "No overview available."],
            expertise: doctor.specializations
                ? doctor.specializations.split(",").map(e => e.trim())
                : [],
            fellowships: doctor.fellowships.map(f => f.title),
            awards: doctor.awards.map(a => a.title),
            experience: doctor.experiences.map(e => e.description),
            memberships: doctor.memberships.map(m => m.name),
            blogs: doctor.blogs.map(b => ({
                date: b.date,
                title: b.title,
